    ...
    <div class="container">
        <div class="row mb-4">
            <div class="col-md-12">
                ...
                <p class="text-muted">Berikut adalah catatan rekam medis dan riwayat percakapan konsultasi yang pernah Anda
                    lakukan di VitaGuard.</p>
            </div>
        </div>
                        ...
                        <tbody>
                                    ...
                                    <td>
                                        @if(in_array($row->status, ['completed', 'selesai', 'success']))
                                            <span class="badge bg-success-subtle text-success px-3 py-2 rounded-pill"><i
                                                    class="bi bi-check-circle-fill"></i> Selesai</span>
                                        @elseif(in_array($row->status, ['proses', 'berlangsung']))
                                            <span class="badge bg-warning-subtle text-warning px-3 py-2 rounded-pill"><i
                                                    class="bi bi-hourglass-split"></i> Berlangsung</span>
                                        @elseif($row->status == 'accepted')
                                            <span class="badge bg-primary-subtle text-primary px-3 py-2 rounded-pill"><i
                                                    class="bi bi-chat-dots-fill"></i> Siap Konsultasi</span>
                                        @elseif($row->status == 'pending')
                                            <span class="badge bg-warning-subtle text-warning px-3 py-2 rounded-pill"><i
                                                    class="bi bi-hourglass-split"></i> Menunggu Dokter</span>
                                        @else
                                            <span class="badge bg-danger-subtle text-danger px-3 py-2 rounded-pill"><i
                            ...
                            @empty                                
                                <tr>
                                    ...
                                    <td><span class="badge bg-success-subtle text-success px-3 py-2 rounded-pill"><i
                                                class="bi bi-check-circle-fill"></i> Selesai</span></td>
                                    <td class="text-center"><button class="btn btn-outline-primary btn-sm rounded-pill px-3"
                                            disabled><i class="bi bi-eye"></i> Detail</button></td>
                                </tr>
                                <tr>
        ...
        <div class="alert alert-info border-0 shadow-sm mt-4 rounded-3 d-flex align-items-center" role="alert">
            ...
            <div class="small">
                <strong>Keamanan Sistem:</strong> Sesuai dengan regulasi rekam medis elektronik, seluruh riwayat konsultasi
                yang telah disimpan bersifat permanen dan **tidak dapat dihapus** oleh pihak pasien demi validitas riwayat
                kesehatan Anda.
            </div>
        </div>
    </div>
